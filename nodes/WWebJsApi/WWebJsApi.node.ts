...
export class WWebJsApi implements INodeType {
  description: INodeTypeDescription = {
    ...
    description:
      "Interact with WhatsApp Web API via WWebJS — send messages, manage sessions, chats, groups, contacts and channels",
    ...
    hints: [
      ...
      {
        message:
          'Output merges input fields with API response. API adds: <code>success</code>, <code>state</code> ("CONNECTED" | null), <code>message</code> ("session_connected" | "session_not_found" | "session_not_connected")',
        type: "info",
        location: "outputPane",
        whenToDisplay: "beforeExecution",
      ...
      {
        message:
          "Output merges input fields with API response. API adds: <code>success</code>, <code>message</code>: the sent Message object — includes <code>message.id</code>, <code>message.from</code>, <code>message.to</code>, <code>message.body</code>, <code>message.timestamp</code>, <code>message.type</code>, …",
        type: "info",
        location: "outputPane",
        whenToDisplay: "beforeExecution",
      ...
      {
        message:
          "Output merges input fields with API response. API adds: <code>success</code>, <code>chats</code>: array of Chat objects — each has <code>id</code>, <code>name</code>, <code>isGroup</code>, <code>isReadOnly</code>, <code>unreadCount</code>, <code>timestamp</code>, <code>archived</code>, <code>pinned</code>, …",
        type: "info",
        location: "outputPane",
        whenToDisplay: "beforeExecution",
      ...
      {
        message:
          "Output merges input fields with API response. API adds: <code>success</code>, <code>contacts</code>: array of Contact objects — each has <code>id</code>, <code>number</code>, <code>name</code>, <code>shortName</code>, <code>pushname</code>, <code>isUser</code>, <code>isGroup</code>, <code>isMyContact</code>, <code>isBlocked</code>, …",
        type: "info",
        location: "outputPane",
        whenToDisplay: "beforeExecution",
      ...
      {
        message:
          "Output merges input fields with API response. API adds: <code>success</code>, <code>info</code>: MessageInfo object with delivery/read receipts — may be null if the message is not sent by you",
        type: "info",
        location: "outputPane",
        whenToDisplay: "beforeExecution",
      ...
      {
        message:
          "Output merges input fields with API response. API adds: <code>success</code>, <code>messages</code>: array of Message objects (sorted earliest → latest) — each has <code>id</code>, <code>body</code>, <code>type</code>, <code>timestamp</code>, <code>from</code>, <code>to</code>, <code>fromMe</code>, <code>hasMedia</code>, …",
        type: "info",
        location: "outputPane",
        whenToDisplay: "beforeExecution",
...
function getMessageContent(executeFn: IExecuteFunctions, i: number) {
  ...
  const textContent = contentType === "string" ? (executeFn.getNodeParameter("content", i, "") as string) : "";
  const jsonContent = contentType !== "string" ? (executeFn.getNodeParameter("contentJson", i, "{}") as string) : "{}";
...
// ── Session ──
async function executeSession(this: IExecuteFunctions, operation: string, i: number): Promise<IDataObject> {
  switch (operation) {
    case "getSessions":
...
// ── Client ──
async function executeClient(this: IExecuteFunctions, operation: string, i: number): Promise<IDataObject> {
  const sid = await resolveSessionId(this, i);
  ...
  switch (operation) {
    case "sendMessage": {
      ...
      if (Object.keys(optionsRaw).length > 0) {
        ...
        if (optionsRaw.caption) {
          options.caption = optionsRaw.caption as string;
        }
    ...
    case "createGroup": {
      ...
      return wpiRequest.call(this, "POST", buildEndpoint("/client/createGroup/{sessionId}", sid), {
        name: groupName,
...
// ── Message ──
async function executeMessage(this: IExecuteFunctions, operation: string, i: number): Promise<IDataObject> {
  const sid = await resolveSessionId(this, i);
  const chatId = getChatId(this, i);
  ...
  switch (operation) {
    ...
    case "reply": {
      const { contentType, content } = getMessageContent(this, i);
      return wpiRequest.call(this, "POST", buildEndpoint("/message/reply/{sessionId}", sid), {
        chatId,
        messageId,
        contentType,
        content,
    ...
    case "forward": {
      ...
      return wpiRequest.call(this, "POST", buildEndpoint("/message/forward/{sessionId}", sid), {
        chatId,
        messageId,
        destinationChatId,
      });
...
// ── Chat ──
async function executeChat(this: IExecuteFunctions, operation: string, i: number): Promise<IDataObject> {
  const sid = await resolveSessionId(this, i);
  const chatId = getChatId(this, i);
...
// ── Group Chat ──
async function executeGroupChat(this: IExecuteFunctions, operation: string, i: number): Promise<IDataObject> {
  const sid = await resolveSessionId(this, i);
  ...
  switch (operation) {
    ...
    case "addParticipants": {
      ...
      return wpiRequest.call(this, "POST", buildEndpoint("/groupChat/addParticipants/{sessionId}", sid), {
        chatId,
        participantIds: ids,
      });
    }
    case "removeParticipants": {
      ...
      return wpiRequest.call(this, "POST", buildEndpoint("/groupChat/removeParticipants/{sessionId}", sid), {
        chatId,
        participantIds: ids,
      });
    }
    case "promoteParticipants": {
      ...
      return wpiRequest.call(this, "POST", buildEndpoint("/groupChat/promoteParticipants/{sessionId}", sid), {
        chatId,
        participantIds: ids,
      });
    }
    case "demoteParticipants": {
      ...
      return wpiRequest.call(this, "POST", buildEndpoint("/groupChat/demoteParticipants/{sessionId}", sid), {
        chatId,
        participantIds: ids,
      });
    }
...
// ── Contact ──
async function executeContact(this: IExecuteFunctions, operation: string, i: number): Promise<IDataObject> {
  const sid = await resolveSessionId(this, i);
  const contactId = getContactId(this, i);
...
// ── Channel ──
async function executeChannel(this: IExecuteFunctions, operation: string, i: number): Promise<IDataObject> {
  const sid = await resolveSessionId(this, i);
  ...
  switch (operation) {
    ...
    case "sendMessage": {
      const chatId = this.getNodeParameter("chatId", i) as string;
      const { contentType, content } = getMessageContent(this, i);
      return wpiRequest.call(this, "POST", buildEndpoint("/channel/sendMessage/{sessionId}", sid), {
        chatId,
        contentType,
        content,
      });
