...
export function validateChatId(chatId: string): ValidationResult {
  ...
  if (!CHAT_ID_REGEX.test(trimmed) && !GROUP_ID_REGEX.test(trimmed) && !CHAT_ID_REGEX_LID.test(trimmed)) {
    return {
      valid: false,
 ...
 */
export function parseContent(contentType: ContentType, textContent: string, jsonContent: string): ParsedContent {
  ...
  try {
    ...
    throw new Error(`Invalid JSON content for type "${contentType}": ${(error as Error).message}`);
  }
}
 ...
 */
export async function resolveSessionId(executeFn: IExecuteFunctions, itemIndex: number): Promise<string> {
